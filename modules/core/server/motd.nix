{
  ...
  monitoredServices = lib.lists.flatten (
    lib.lists.forEach enabledNixosServices (
      x: let
        svc = config.homeserver.services.${x};
      in
  ...
  networkInterface =
    if
      (config ? homeserver
        && config.homeserver ? networks
        && config.homeserver.networks ? external
        && lib.attrsets.hasAttrByPath [config.networking.hostName] config.homeserver.networks.external)
    then config.homeserver.networks.external.${config.networking.hostName}.interface
    else "";
  motd = pkgs.writeShellScriptBin "motd" ''
    ...
    ${lib.strings.concatMapStrings (x: "${x}\n") (
      lib.lists.forEach config.homeserver.motd.networkInterfaces (
        x:
          lib.strings.concatMapStrings (x: "${x}\n") [
            (
              if x == ""
...
in {
  ...
  config = lib.mkIf config.homeserver.motd.enable {
